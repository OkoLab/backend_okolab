<?php

namespace App\Services\MoySklad;

use App\Services\MoySklad\Actions\GetInvoiceoutAction;
use App\Services\MoySklad\Data\CounterpartyData;
use App\Services\MoySklad\Entities\InvoiceoutEntity;
use App\Services\MoySklad\Parser\IParser;
use Illuminate\Support\Collection;

class MoySkladService
{
    public function __construct(
        public readonly MoySkladConfig $config,
        public readonly IParser $parser
    ) {

    }

    /**
     * @param string $inputString
     * @return Collection<int, InvoiceoutEntity>
     */
    public function getEntityCollectionFromString(string $inputString): Collection
    {
        return $this->getEntityCollection($inputString);
    }

    public function getEntityCollection(string $inputString): Collection
    {
        $dataCollection = $this->parser->parseFromString($inputString);

        return collect($dataCollection)->map(function (CounterpartyData $counterpartyData) {
                $entity = GetInvoiceoutAction::make($this)
                ->findByName($counterpartyData)
                ->getAssortiments()
                ->run();

                return $entity;
        });
    }




    // public function createInvoiceoutEntity(string $cmd): InvoiceoutEntity
    // {
    //     $counterpartyData = new CounterpartyString($cmd);
    //     return GetInvoiceoutAction::make($this)
    //         ->findByName($counterpartyData)
    //         ->getAssortiments()
    //         ->run();
    // }

}
